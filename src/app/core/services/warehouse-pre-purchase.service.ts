...
export class ServiceWarehousePrePurchase {
  ...
  uploadSheet(request: any) {
    return this.http.post(
      AppConstants.SERVER_URL + ApiConstants._WAREHOUSE_PRE_PURCHASES.POST.UPLOAD_SHEET,
      request
    );
  }
  ...
  getPurchaseList(page: DtPage) {
    ...
    return this.http.post(
      AppConstants.SERVER_READONLY_URL + ApiConstants._WAREHOUSE_PRE_PURCHASES.POST.PRE_PURCHASE_LIST,
      page
    );
  }
  ...
  getPurchaseInfo(purchaseId: number) {
    ...
    let url = AppConstants.SERVER_READONLY_URL +
      ApiConstants._WAREHOUSE_PRE_PURCHASES.GET.PRE_PURCHASE_INFO + purchaseId + '/' + langId;
    console.log(url);
  ...
  onApprove(purchaseId: number, isApproved: number) {
    return this.http.get(
      AppConstants.SERVER_READONLY_URL +
      ApiConstants._WAREHOUSE_PRE_PURCHASES.GET.PRE_PURCHASE_APPROVE + purchaseId + '/' + isApproved
    );
  }
  ...
  onOpen(purchaseId: number, open: number) {
    return this.http.get(
      AppConstants.SERVER_READONLY_URL +
      ApiConstants._WAREHOUSE_PRE_PURCHASES.GET.PRE_PURCHASE_OPEN + purchaseId + '/' + open
    );
  }
